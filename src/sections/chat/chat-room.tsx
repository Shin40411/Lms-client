...
export function ChatRoom({ collapseNav, participants, messages, loading, type, idOwner, sx, ...other }: Props) {
  ...
  const renderContent = () =>
    loading ? (
      ...
      <Scrollbar>
        <div>
          {isGroup ? (
            <ChatRoomGroup participants={participants} idOwner={idOwner} />
          ) : (
            <ChatRoomSingle participant={participants[0].user} />
          )}
